...
""" File name:   hawkweed_simulation.py
    Author:      COMP3620/6320 Team - ANU
    Date:        2021
    Description: This is the main file for COMP3620 Assignment 0 Exercise 4.

                 --- You do not need to modify this file ---

                 python hawkweed_simulation.py to get usage information.
"""
...
test_params = [
    ...
    {"threshold": 1, "growth": 1, "spread": 1,
     ...
     "location": "l9",
     "hawkweed": {"l0": 0.5, "l1": 0, "l2": 0, "l3": 0, "l4": 0, "l5": 0, "l6": 0,
                 "l7": 0,   "l8": 0, "l9": 0},
...
def test_scenario_file(module):
    """ Test the given hawkweed_scenario module.

        (module) -> None
    """
    print("Testing implementation of hawkweed_scenario.py")
    print("--------------------------------------------------------------------")

    print("Checking for HawkweedScenario class...")
    print("-------------------------------------")
    try:
        ds_class = module.HawkweedScenario
        if type(ds_class) != type:
            print("Failed. HawkweedScenario is not a class.")

            class tc:
                pass
            if type(ds_class) == type(tc):
                print("You have forgotten to make HawkweedScenario subclass object.")
            raise TestingError()
        ds = ds_class()
    except AttributeError:
        print("Failed. HawkweedScenario does not exist.")
        raise TestingError()
    print("Success.")
    ...
    try:
        ...
        for test, tparams in zip(test_files, test_params):
            ...
            try:
                if not isinstance(ds_copy.hawkweed, dict):
                    print("Error: hawkweed is not a dictionary.")
                    raise TestingError()
                for loc, dis in ds_copy.hawkweed.items():
                    if loc not in tparams["hawkweed"]:
                        print("Unexpected hawkweed location:", loc)
                        raise TestingError()

                    if not isinstance(dis, float) and not isinstance(dis, int):
                        print("Error: the hawkweed at location",
                              loc, "should be a float or int!")
                        raise TestingError()

                    if abs(tparams["hawkweed"][loc] - dis) > 0.001:
                        print("Invalid hawkweed at location:", loc, "expected:",
                              tparams["hawkweed"][loc])
                        raise TestingError()
                for loc, dis in tparams["hawkweed"].items():
                    if loc not in ds_copy.hawkweed:
                        print(
                            "Missing hawkweed location (locations should have 0 hawkweed by default):", loc)
                        raise TestingError()
            except AttributeError:
                print("Error: hawkweed variable is missing.")
                raise TestingError()
    ...
    for tid, test_file in enumerate(test_files):
        ...
        try:
            ...
            print(traceback.format_exc())
            raise TestingError()
        if ds.hawkweed[tparams["vloc"]] != 0:
            print("Error: moving to", tparams["vloc"],
                  "did not clear hawkweed there in test:", test_file)
            raise TestingError()
    ...
    print("Testing spread_hawkweed method.")
    print("-------------------------------------")
    try:
        sd = ds.spread_hawkweed
    except AttributeError:
        print("Failed. spread_hawkweed does not exist.")
        raise TestingError()
    ...
    for tid, test_file in enumerate(test_files):
        ...
        try:
            ...
            print("Unexpected error when running spread_hawkweed for test:", test_file)
            print(traceback.format_exc())
            raise TestingError()
        ...
        try:
            ...
            for loc in tparams["locations"]:
                if loc == tparams["location"]:
                    if ds.hawkweed[loc] != 0:
                        print("Error: we expect there to always be zero hawkweed",
                              "at the current location:", test_file, "instead there is:",
                              ds.hawkweed[loc])
                        raise TestingError()
                else:
                    if abs(ds.hawkweed[loc] - ed[loc]) > 0.01:
                        print("Error: after 10 iterations there is the wrong",
                              "hawkweed", ds.hawkweed[loc], "expected", ed[loc],
                              "(0.01 epsilon) at location:", loc, "test:", test_file)
                        raise TestingError()
        except AttributeError:
            print("Error: you must have deleted hawkweed when running spread_hawkweed",
                  "for test:", test_file)
...
def print_summary(scenario):
    """ Print a summary of the hawkweed spread and current location in the
        given scenario.

        (HawkweedScenario) -> None
    """
    print("The locations have the following hawkweed:")
    total_hawkweed = 0
    tokens = []
    for loc in scenario.locations:
        hawkweed = scenario.hawkweed[loc]
        total_hawkweed += hawkweed
        if hawkweed >= scenario.threshold:
            status = "(spreading)"
        else:
            status = "()"
        tokens.append(' '.join([loc, str(hawkweed), status]))
    print(','.join(tokens))
    print("Total hawkweed:", total_hawkweed)
    return total_hawkweed


def show_graph(graph):
    graph.graph = nx.Graph()
    for node, hawkweed in graph.hawkweed.items():
        graph.graph.add_node(node, label=node + ": " + str(hawkweed))
    ...
    nx.draw(graph.graph, pos=nx.circular_layout(graph.graph), with_labels=True, font_weight='bold',
            labels={k: "{}\n{:.2f}".format(k, v)
                    for k, v in graph.hawkweed.items()},
            node_size=1300,
            node_color=["b" if loc == graph.location else "g" if d <= 0 else "r" for loc, d in graph.hawkweed.items()])
    plt.text(-1, 0.8,
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description='Horseman No. 2 - a hawkweed simulator')
    parser.add_argument('-t', '--test', default=False, action='store_true',
                        help='Activates TESTING mode (default: deactivated)')
    parser.add_argument('-s', '--scenario', default=None,
                        type=str, help='Path to scenario (.scn) file')
    parser.add_argument('-a', '--agent', default='HawkweedEradicationAgent',
                        type=str, help='Agent to be used (default: HawkweedEradicationAgent)')
    ...
    parser.add_argument('-v', '--viz', default=False, action='store_true',
                        help='Draw a graph to visualise how hawkweed is spread. '
...
def main():
    ...
    try:
        with Capturing() as output:
            import hawkweed_scenario
    except ImportError:
        print("Error hawkweed_scenario.py does not exist.")
        sys.exit()
    except Exception:
        print("Error: exception when importing hawkweed_scenario:")
        print(traceback.format_exc())
        sys.exit()

    if output:
        print("Error hawkweed_scenario.py produced output when imported.")
        sys.exit()

    if args.test:
        try:
            test_scenario_file(hawkweed_scenario)
        except TestingError:
            pass
    else:
        try:
            ds = hawkweed_scenario.HawkweedScenario()
            ...
            print(
                "Unexpected error while loading the specified hawkweed scenario:", args.scenario)
            print(traceback.format_exc())
            print("Are you sure you tested hawkweed_scenario correctly?")
            sys.exit()

        agent_name = args.agent
        print("Trying to load agent:", agent_name)
        try:
            agent_module = importlib.import_module("hawkweed_eradication_agents")
            base_class = agent_module.HawkweedEradicationAgent
        except ImportError:
            print("Error loading agent from hawkweed_eradication_agents.py")
            print("Make sure the file exists.")
            sys.exit()
        except Exception:
            print("Unexpected error while trying to import hawkweed_eradication_agents.py:")
            print(traceback.format_exc())
        ...
        if agent_name not in agent_module.__dict__:
            print("There is no such agent as",
                  agent_name, "in hawkweed_eradication_agents.py")
            sys.exit()

        agent_class = agent_module.__dict__[agent_name]
        if not issubclass(agent_class, base_class):
            print("Error:", agent_name, "is not an instance of a HawkweedEradicationAgent.")
            sys.exit()
            ...
            for j in range(0, args.num_sims):
                ...
                for i in range(iterations):
                    ...
                    move = agent.choose_move(ds.location, list(valid_moves),
                                             dict(ds.hawkweed), ds.threshold, ds.growth, ds.spread)

                    print("Selected move:", move)

                    ds.move(move)
                    ds.spread_hawkweed()
                    total_hawkweed = print_summary(ds)

                    if args.viz:
                        show_graph(ds)

                    if total_hawkweed < 0.001:
                        print("You eradicated all hawkweed! Good job.")
                        break
                    if total_hawkweed >= 100000:
                        print("Total hawkweed went over 100000.")
                        print("Hawkweed has taken over Kosciuszko National Park and its surroundings,")
                        print("and has driven native flora and fauna to extinction.")
                        break
                sim_data = {}
                sim_data['last_step'] = i
                sim_data['final_hawkweed'] = total_hawkweed
                sim_data['score'] = 0.0
                if total_hawkweed < 100000:
                    if i < args.horizon - 1:
                        ...
                        sim_data['score'] = 1.0 - \
                            float(total_hawkweed)/float(100000)
